      ...
      var $dom = $(this);
        ...
        if (options !== undefined && (typeof(options) == "boolean" || typeof(options) == "string")) { // Force flip the DOM
          ...
          var rotateAxis = "rotate" + (settings.axis.toLowerCase() == "x" ? "x" : "y"), 
              perspective = 200;//$dom["outer" + (rotateAxis == "rotatex" ? "Height" : "Width")]() * 2;
          ...
          if (settings.forceHeight) {faces.outerHeight($dom.height());} else if (settings.autoSize) {faces.css({'height': '150px'});}
          if (settings.forceWidth) {faces.outerWidth($dom.width());} else if (settings.autoSize) {faces.css({'width': '100px'});}
          faces.css({
            "backface-visibility": "hidden",
